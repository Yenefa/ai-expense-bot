...
- 多轮用例通过 gold `previous_route` 模拟会话上下文（v3.9.3 起）；真实 App 中该上下文由 `ConversationActionContext` 在会话内推进
...
### v3.9.2 本地加固结果（2026-09-11，纯本地，无 LLM）

- 已完成（owner 指定四项）：**CHAT write firewall**（非 MUTATION 路径代码层禁写）、**Router 收敛**（裸金额→MUTATION；非支出护栏→CHAT；查询追问召回）、**纯日期 `occurred_at` 容错**、**partial hints**
- 离线基线：前置路由 58/80 → **74/80**；Query 前置召回 9/16 → **14/16**；工具选择 14/14；JVM 288/288
- 防火墙证明：闲聊幻觉 + 查询注入两类用例下 `applyPlan` 零调用、数据库零修改（正文回归测试）
- **LLM Bench 暂停**（owner 2026-09-11 决定）：不再自动三轮全量；后续有免费额度/替代模型/明确指令时先跑 smoke，再决定是否全量
- 仍未做（后续候选）：分类灰区 gold 复核

### v3.9.3 会话上下文结果（2026-09-11，纯本地）
