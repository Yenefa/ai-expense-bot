...
| `anomalous_spending` | 本周消费（或某一分类／商户）≥ 近 4 个完整周基线均值 × 1.5，且高出 ≥ ¥100；总量正常时按分类级、分类也正常时按商户级（note）逐级兜底（同级多个异常只报偏离金额最大的一个） | 4 个可比样本 |
...
- 异常检测：分类级（v3.14）与商户级（v3.15，按 `note`）基线均已支持，层级固定为 **总量 > 分类 > 商户**；商户按备注文本精确匹配（同一商户不同写法不合并、空备注不参与），文案中超过 12 字截断展示
- 储蓄外推用线性 pace（含"剩余 N 天最多还能花"细化），不区分工作日/节假日
- 调度语义：设备重启后 WorkManager 自动恢复；但用户在系统里**强制停止（force-stop）**本应用后，所有 WorkManager 任务会暂停，直到下一次手动打开 App 才恢复（Android 平台行为）。周期账单在下次启动时会补跑，主动提醒按治理额度补齐
