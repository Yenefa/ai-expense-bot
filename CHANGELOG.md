...
- **契约进 CI**：新增 `BackupPolicyTest`（JVM）——云备份必须排除财务数据与凭据、设备迁移必须保留账目与预算、清单必须声明两套规则；`tools/test-api-key-backup-rules.ps1` 补**反向断言**（原先只校验「该排除的都在」，没有「不该排除的不能被加回来」）
...
测试：402 → **405 个 JVM 单测全部通过**（+3 备份契约）；`lintDebug` 通过；`tools/test-api-key-backup-rules.ps1` PASS（已用负向对照验证能拦住回归）。
