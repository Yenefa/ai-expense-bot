...
## 3.15.3 (versionCode 50) — 2026-09-15

**修复：官方账单 CSV 导入（owner 真机报障，三个独立缺陷）**

- **根因**：v3.14.2 为支持官方账单的「前置元信息行」加入了入口判据 `looksLikePlatformCsv`，但它只扫描**前 15 行**；而微信官方账单的导出信息 + 笔数统计 + 注意事项通常**超过 15 行**，于是判据返回 false，文件被回落到「本应用导出 CSV」解析器，报出 `CSV 缺少字段：id, amount, categoryId, …` —— 用户看到的就是「账单不支持导入」
- **修复**：入口判据改为**扫描全文**寻找表头，与 `parse` 的定位逻辑一致（判据绝不能再比解析器更窄）；顺带删除失效的 `MARKER_SCAN_LINES`
- **同类隐患一并修**：CSV 导入的文件选择器只放行 `text/*` / `application/csv` / `application/vnd.ms-excel`，而官方账单经下载或文件管理器落盘后常被 provider 报成 `application/octet-stream` → 文件在选择器里**变灰不可选**。已补 `application/octet-stream` 与 `application/x-csv`

**缺陷 2：CSV 引号处理不符合 RFC 4180 —— 一个落单引号就废掉整份账单**

- **现象**（owner 真机截图）：`导入失败：CSV 存在未闭合的引号（第 256 行）`
- **根因**：解析器把**任何位置**的 `"` 都当作引号开关。RFC 4180 只允许**字段开头**的 `"` 起始引用，字段中间出现的 `"` 是普通字符；备注 / 商品名里只要有一个**落单**引号，状态机就一路错位到文件末尾，整份账单被拒绝导入
- **修复**：`"` 仅在当前字段为空时起始引用，否则按字面量追加

**缺陷 3：压缩包只会得到风马牛不相及的错误提示**

- `decodeCsvBytes` 识别 ZIP 魔数（`PK` 开头）→ 明确提示「这是压缩包（.zip），请先解压出 CSV 再导入」，而不是让用户看到「缺少字段」
- 顺带把编码回退（UTF-8 → GBK）从 UI 层挪进 `CsvExpenseImporter`，**首次获得单元测试覆盖**

测试：406 → **410 个 JVM 单测全部通过**（+4：长前置行 1 / 落单引号 1 / ZIP 识别 1 / GBK 回退 1）；`lintDebug` 通过。

> 说明：微信「下载账单」与支付宝「开具交易流水证明」的邮件/消息附件可能是**加密 ZIP**，需先解压出 CSV 再导入；App 现在会**明确提示是压缩包**，但仍不解压（如需内置解压请另行提需求）。

## 3.15.2 (versionCode 49) — 2026-09-14

**分类口径修正：食材/生鲜/蔬菜/水果/买菜 → food（owner 裁定）**

- **补分类规则**：`LlmPrompt.systemPrompt` 与 `LlmPrompt.billImportSystemPrompt` 新增「食材、生鲜、蔬菜、水果、买菜 → food」，并显式消歧「从超市买的食材也算 food，只有日用品/生活用品才归 shopping」
- **根因**：提示词里此前只有「日用品、超市……→ shopping」，没有任何食材类指引，模型据此把「买菜」「水果」归到购物
- **来源**：ExpenseBench v2 旁路基线（deepseek-v4.1-flash，3 轮 × 110 条）唯一残留失败根因（ra-08 / mtp-17）；修复后复测 **3 轮 E2E 全部 100%（110/110）**，修复前为 99.1%（109/110）
- **契约**：`LlmPromptContractTest` 新增断言，钉住两个提示词的食材分类规则

测试：405 → **406 个 JVM 单测全部通过**；`lintDebug` 通过。**未改动**评测器、数据集 gold、阈值与冷却。

## 3.15.1 (versionCode 48) — 2026-09-13

**隐私边界收紧：财务数据不再进入系统云备份（owner 决策 C）**

- **账目/聊天/周期账单不上传云备份**：`expense.db`（含 `-wal` / `-shm` / `-journal` 边车文件）加入排除项，Google 云备份不再包含记账与聊天数据
- **预算同口径排除**：`budget_prefs` 属财务数据，一并排除
- **换机体验保留（Android 12+）**：`device-transfer` 不排除账目与预算——设备对设备直传、不经服务器；Android 8–11 因平台限制（`fullBackupContent` 云备份与设备迁移共用同一套规则）不再自动迁移账目与预算，请使用 App 内「完整备份（JSON）」
- **契约进 CI**：新增 `BackupPolicyTest`（JVM）——云备份必须排除财务数据与凭据、设备迁移必须保留账目与预算、清单必须声明两套规则；该测试是这条边界的**唯一权威**——原先只存在于 `tools/test-api-key-backup-rules.ps1` 的清单已收编为薄封装（脚本只调用本测试，不再维护第二份期望），并补上了原先缺失的**反向断言**（「不该排除的不能被加回来」）
- **文档三处同步**：README 数据安全节、App 内置说明书「数据安全」、官网隐私卡

> 行为影响：重装/换新机不再从云备份恢复账目（此前会恢复账目，但长期记忆与凭据本就不恢复，属「部分恢复」）。同一签名的正常升级不受影响；换机请使用 App 内「完整备份（JSON）」。

测试：402 → **405 个 JVM 单测全部通过**（+3 备份契约）；`lintDebug` 通过；`tools/test-api-key-backup-rules.ps1`（薄封装调用同一测试）PASS；已用**负向对照**验证契约能拦住回归。
