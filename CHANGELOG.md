...
## 3.15.3 (versionCode 50) — 2026-09-14

**修复：官方账单 CSV「不支持导入」（v3.14.2 引入的检测窗口缺陷）**
...
测试：406 → **407 个 JVM 单测全部通过**（+1：超过旧窗口的长前置行回归）；`lintDebug` 通过。

> 说明：微信「下载账单」与支付宝「开具交易流水证明」邮件附件可能是**加密 ZIP**，需先解压出 CSV 再导入；App 目前不解压（如需支持请另行提需求）。
