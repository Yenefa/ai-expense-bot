...
object LlmPrompt {
    ...
    fun systemPrompt(nowMillis: Long = System.currentTimeMillis()): String {
        ...
        return buildString {
            ...
            appendLine("【正确示例】")
            appendLine("用户：你好")
            appendLine("""你的输出：{"reply":"你好呀😊 今天有什么记账的吗？","expenses":[]}""")
            appendLine()
            appendLine("用户：午饭35")
            appendLine("""你的输出：{"reply":"已记 ¥35 餐饮","expenses":[{"amount":35,"category":"food","note":"午饭","occurred_at":null}]}""")
            appendLine()
            appendLine("【规则】")
            ...
            appendLine("7. 投资类（买股票/买基金/做短线/打新等）使用 category=\"investment\"。这类支出会被排除出消费分析图表，不影响日常消费洞察。")
            appendLine()
            appendLine("【分类对照】")
