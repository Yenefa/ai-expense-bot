...
（规则不命中但句子带    直接执行   确认门（Human Confirm）
 ...
 query_expenses / analyze_expenses / get_budget_status
       │
 工具结果注入 system prompt → LLM 只负责组织语言
```
...
```
用户表达 → Memory Proposal → 类型校验 → 预览 → Human Confirm → UserProfile
```
...
| **v3.14.1** | **深度审计修复** - 历史页跨年崩溃；周期账单跳月/31 号漂移/并发重复；未来日期不计入本月/今天；提醒页配置不再被默认值覆盖；坏备份整包拒绝；LLM 多候选 JSON 解析；会员页轮询随页面停止；Locale 固定；正式签名基础设施与 CI 加固 |
| **v3.14.2** | **第二轮深度审计修复** - 截图导入无 AI 配置不再崩溃；真实平台账单（含前置信息行）可导入；续记日期按消息时间解析；指代词查询不再误入写路径；LLM 请求可取消/超时/重试/限长；OCR 位图回收；DataStore 抗损坏；Keystore 失效防崩；口语金额（23块5/3块半）；Room 索引迁移；软删后可重新导入 |
| **v3.15.0** | **商户级异常基线** - 总量与分类都正常时，单一商户（note）本周 ≥ 近 4 周均值 ×1.5 且高出 ≥¥100 也会提醒；层级固定 总量 > 分类 > 商户，多商户只报偏离最大的一个；≥4 可比样本 / 冷启动 / 冷却 / 开关约束照旧 |
| **v3.15.1** | **隐私边界收紧** - 账目与聊天记录（`expense.db` 及 WAL 边车）、预算移出系统云备份；Android 12+ 保留设备间迁移；新增 CI 契约测试 `BackupPolicyTest`；文档三处同步 |
| **v3.15.2** | **分类口径修正** - 「食材/生鲜/蔬菜/水果/买菜」明确归 food（此前提示词只写了「超市 → shopping」，模型把买菜/水果归到购物）；主链路与账单导入提示词同步；新增提示词契约断言 |
| **v3.15.3** | **修复官方账单 CSV 导入** - v3.14.2 的入口判据只扫前 15 行，微信账单前置说明常超 15 行 → 被误判为「非平台账单」并回落到本应用 CSV 解析器（报「CSV 缺少字段」）；改为扫描全文。文件选择器补 `octet-stream` / `x-csv`，账单被报成通用类型时不再变灰不可选 |
