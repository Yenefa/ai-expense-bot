...
- 按日期分组，展开看当天每一笔
- 分类、金额、时间一目了然
- **消费日历（v1.4 新增）**：周 / 月 / 年三种方格视图，每格显示消费总额和笔数，点击格子查看明细
...
- **v1.4.0** (2026-06-16) — **消费日历 + 版本统一**：历史页新增周/月/年方格日历，Android `versionName/versionCode` 对齐 APK 版本
- **v1.4.1** (2026-06-16) — 聊天查询卡片升级为统计卡片；模板消息去掉 `[模板]` 前缀
- **v1.4.2** (2026-06-16) — 查询统计卡片移除“知道了”按钮，结果常驻显示
- **v1.4.3** (2026-06-16) — 聊天气泡长按复制；API Key 改用 Android Keystore + EncryptedSharedPreferences 加密存储，自动迁移旧数据
